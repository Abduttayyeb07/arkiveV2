...
from arkive.internal.db import Base, get_db
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(ARKIVE_DIR / 'alembic.ini')

        # Set the script location dynamically
        migrations_path = ARKIVE_DIR / 'migrations'
...
if JWT_EXPIRES_IN.value == '-1':
    log.warning(
        ...
        '    See: https://github.com/Abduttayyeb07/ArkiveV2IA/reference/env-configuration\n'
    )
...
STATIC_DIR = Path(os.getenv('STATIC_DIR', ARKIVE_DIR / 'static')).resolve()
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (Arkive + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.arkive.svc.cluster.local:11434'
...
ARKIVE_URL = PersistentConfig('ARKIVE_URL', 'arkive.url', os.environ.get('ARKIVE_URL', ''))


ENABLE_SIGNUP = PersistentConfig(
    'ENABLE_SIGNUP',
    'ui.enable_signup',
    (False if not ARKIVE_AUTH else os.environ.get('ENABLE_SIGNUP', 'True').lower() == 'true'),
)
...
try:
    banners = json.loads(os.environ.get('ARKIVE_BANNERS', '[]'))
    banners = [BannerModel(**banner) for banner in banners]
except Exception as e:
    log.exception(f'Error loading ARKIVE_BANNERS: {e}')
    banners = []

ARKIVE_BANNERS = PersistentConfig('ARKIVE_BANNERS', 'ui.banners', banners)
...
MILVUS_COLLECTION_PREFIX = os.environ.get('MILVUS_COLLECTION_PREFIX', 'arkive')
...
QDRANT_COLLECTION_PREFIX = os.environ.get('QDRANT_COLLECTION_PREFIX', 'arkive')
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get('ELASTICSEARCH_INDEX_PREFIX', 'arkive_collections')
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'arkive-index')
