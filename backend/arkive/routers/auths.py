...
from arkive.models.auths import (
    AddUserForm,
    ApiKey,
    Auths,
    Token,
...
from arkive.models.users import (
    UserModel,
...
from arkive.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
from arkive.env import (
    ARKIVE_AUTH,
    ARKIVE_AUTH_TRUSTED_EMAIL_HEADER,
    ARKIVE_AUTH_TRUSTED_NAME_HEADER,
    ARKIVE_AUTH_TRUSTED_GROUPS_HEADER,
    ARKIVE_AUTH_TRUSTED_ROLE_HEADER,
    ARKIVE_AUTH_COOKIE_SAME_SITE,
...
from arkive.utils.misc import parse_duration, validate_email_format
...
from arkive.utils.access_control import get_permissions, has_permission
from arkive.utils.groups import apply_default_group_assignment

from arkive.utils.redis import get_redis_client
from arkive.utils.rate_limit import RateLimiter
...
def create_session_response(request: Request, user, db, response: Response = None, set_cookie: bool = False) -> dict:
    ...
    if set_cookie and response:
        ...
        response.set_cookie(
            ...
            httponly=True,
            samesite=ARKIVE_AUTH_COOKIE_SAME_SITE,
            secure=ARKIVE_AUTH_COOKIE_SECURE,
...
async def update_password(
    ...
    if ARKIVE_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(
    ...
    if ARKIVE_AUTH_TRUSTED_EMAIL_HEADER:
        if ARKIVE_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[ARKIVE_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if ARKIVE_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(ARKIVE_AUTH_TRUSTED_NAME_HEADER, email)
            try:
        ...
        user = Auths.authenticate_user_by_email(email, db=db)
        if user:
            if ARKIVE_AUTH_TRUSTED_GROUPS_HEADER:
                group_names = request.headers.get(ARKIVE_AUTH_TRUSTED_GROUPS_HEADER, '').split(',')
                group_names = [name.strip() for name in group_names if name.strip()]

                if group_names:
                    Groups.sync_groups_by_group_names(user.id, group_names, db=db)

            if ARKIVE_AUTH_TRUSTED_ROLE_HEADER:
                trusted_role = request.headers.get(ARKIVE_AUTH_TRUSTED_ROLE_HEADER, '').lower().strip()
...
async def signout(request: Request, response: Response, db: Session = Depends(get_session)):
                            ...
                            if logout_url:
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f'&post_logout_redirect_uri={ARKIVE_AUTH_SIGNOUT_REDIRECT_URL}'
                                            if ARKIVE_AUTH_SIGNOUT_REDIRECT_URL
                                            else ''
                                        ),
                                    },
                                    headers=response.headers,
    ...
    if ARKIVE_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                'status': True,
                'redirect_url': ARKIVE_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def get_admin_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        'ADMIN_EMAIL': request.app.state.config.ADMIN_EMAIL,
        'ARKIVE_URL': request.app.state.config.ARKIVE_URL,
...
async def update_admin_config(request: Request, form_data: AdminConfig, user=Depends(get_admin_user)):
    ...
    request.app.state.config.ARKIVE_URL = form_data.ARKIVE_URL
...
async def token_exchange(
    ...
    """
    Exchange an external OAuth provider token for an Arkive JWT.
