...
from arkive.models.channels import (
    Channels,
    ChannelModel,
    ChannelForm,
    ChannelResponse,
...
from arkive.models.access_grants import AccessGrants, has_public_read_access_grant, has_public_write_access_grant
from arkive.models.messages import (
    Messages,
    MessageModel,
    MessageResponse,
...
from arkive.utils.channels import extract_mentions, replace_mentions
...
async def send_notification(request, channel, message, active_user_ids, db=None):
    name = request.app.state.ARKIVE_NAME
            ...
            if enable_user_webhooks and user.settings:
                ...
                if webhook_url:
                    await post_webhook(
                        name,
                        webhook_url,
                        f'#{channel.name} - {arkive_url}/channels/{channel.id}\n\n{message.content}',
                        {
                            'action': 'channel',
                            'message': message.content,
                            'title': channel.name,
                            'url': f'{arkive_url}/channels/{channel.id}',
                        },
                    )
