"""OpenTelemetry metrics bootstrap for Arkive.
...
`/metrics` endpoint – Arkive does **not** expose it directly.
...
def _build_meter_provider(resource: Resource) -> MeterProvider:
    ...
    views: List[View] = [
        ...
        View(
            instrument_name='arkive.users.total',
        ),
        View(
            instrument_name='arkive.users.active',
        ),
        View(
            instrument_name='arkive.users.active.today',
        ),
    ]
...
def setup_metrics(app: FastAPI, resource: Resource) -> None:
    ...
    meter.create_observable_gauge(
        name='arkive.users.active.today',
