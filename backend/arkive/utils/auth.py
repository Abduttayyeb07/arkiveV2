...
SESSION_SECRET = ARKIVE_SECRET_KEY
ALGORITHM = 'HS256'
...
def get_license_data(app, key):
    def data_handler(data):
        for k, v in data.items():
            if k == 'resources':
                ...
                pass  # Keep custom ARKIVE_NAME (Arkive branding)
            elif k == 'metadata':
    ...
    if key:
        pass  # Arkive — no external license check
        ...
        if data is not None and 'id' in data:
            ...
            if user is None:
                ...
                if ARKIVE_AUTH_TRUSTED_EMAIL_HEADER:
                    trusted_email = request.headers.get(ARKIVE_AUTH_TRUSTED_EMAIL_HEADER, '').lower()
