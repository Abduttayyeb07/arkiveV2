...
from arkive.models.auths import Auths
from arkive.models.oauth_sessions import OAuthSessions
from arkive.models.users import Users


from arkive.models.groups import Groups, GroupModel, GroupUpdateForm, GroupForm
...
from arkive.utils.misc import parse_duration
from arkive.utils.auth import get_password_hash, create_token
...
from arkive.env import GLOBAL_LOG_LEVEL
...
async def get_oauth_client_info_with_dynamic_client_registration(
    ...
    try:
        ...
        redirect_base_url = (str(request.app.state.config.ARKIVE_URL or request.base_url)).rstrip('/')
...
class OAuthClientManager:
    ...
    async def handle_callback(self, request, client_id: str, user_id: str, response):
        ...
        redirect_url = (str(request.app.state.config.ARKIVE_URL or request.base_url)).rstrip('/')
...
class OAuthManager:
    ...
    def update_user_groups(self, user, user_data, default_permissions, db=None):
        ...
        log.debug(f'All groups available in Arkive: {[g.name for g in all_available_groups]}')
